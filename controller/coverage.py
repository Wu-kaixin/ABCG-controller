...
from .boundary import BoundaryEstimateV2
from .arclength import max_consecutive_arc_gap
...
def boundary_demand(boundary: BoundaryEstimateV2, observation: Array,
                    weights: Array, bandwidth: float) -> Array:
    """Gaussian interpolation of observed individual demand, separate from confidence."""
    ...
    if points.ndim != 2 or points.shape[1] != 2 or not len(points) or not np.isfinite(points).all():
        raise ValueError("observation must be finite nonempty (N, 2)")
    if values.shape != (len(points),) or not np.isfinite(values).all() or np.any(values <= 0):
        raise ValueError("demand weights must be finite positive and match observations")
    if not np.isfinite(bandwidth) or bandwidth <= 0:
        raise ValueError("bandwidth must be finite and positive")
    squared = np.sum((boundary.offset_points[:, None] - points[None, :])**2, axis=2)
    squared -= squared.min(axis=1, keepdims=True)
    kernel = np.exp(-squared / (2 * bandwidth**2))
    return (kernel @ values) / kernel.sum(axis=1)


def plan_weighted_arc_coverage(boundary: BoundaryEstimateV2, m: int, density: Array,
                               config: PeriodicArcCVTConfig) -> CoveragePlan:
    """Discrete periodic Lloyd extension for heterogeneous deployment demand.

    Uses fixed quadrature nodes on the deployment curve and explicitly reports
    its discretized cost. No continuous optimality or global convergence claim.
    """
    count = _validate_agent_count(m)
    if not _boundary_contract_valid(boundary):
        return _invalid_plan(boundary, "PLAN_INVALID_BOUNDARY", "invalid_boundary_contract")
    rho = np.asarray(density, dtype=float)
    if rho.shape != boundary.arc_s.shape or not np.isfinite(rho).all() or np.any(rho <= 0):
        raise ValueError("density must be finite positive and match boundary samples")
    # Avoid ambiguous antipodal unwrapping for one-site coverage.
    if count < 2:
        raise ValueError("weighted periodic planning requires at least two guides")
    length = boundary.length
    q_count = max(2048, count * 128, len(rho) * 4)
    q = (np.arange(q_count) + 0.5) * length / q_count
    mass = _interpolate_periodic(rho, boundary.arc_s, length, q) * length / q_count
    sites = equal_arc_target_s(length, count)
    history = []
    converged = False
    status = "PLAN_MAX_ITERATIONS"

    def cells(s):
        delta = (q[:, None] - s[None, :] + length/2) % length - length/2
        owner = np.argmin(np.abs(delta), axis=1)
        nearest = delta[np.arange(q_count), owner]
        return owner, nearest, float(np.sum(mass * nearest**2))

    owner, delta, cost = cells(sites)
    history.append(cost)
    for _ in range(config.max_iterations):
        cell_mass = np.bincount(owner, weights=mass, minlength=count)
        if np.any(cell_mass <= 0):
            return _invalid_plan(boundary, "PLAN_INVALID_EMPTY_CELL", "empty_weighted_quadrature_cell")
        move = np.bincount(owner, weights=mass * delta, minlength=count) / cell_mass
        confidence = _interpolate_periodic(boundary.confidence, boundary.arc_s, length, sites)
        move *= np.maximum(config.eta_min, confidence)
        proposed = np.sort((sites + move) % length)
        next_owner, next_delta, next_cost = cells(proposed)
        if next_cost > cost + config.monotonic_tolerance:
            status = "PLAN_INVALID_H_INCREASE"
            break
        improvement = cost - next_cost
        sites, owner, delta, cost = proposed, next_owner, next_delta, next_cost
        history.append(cost)
        if np.max(np.abs(move)) <= config.target_tolerance or improvement <= config.h_tolerance * max(1.0, cost):
            converged, status = True, "VALID"
            break
    bounds, _, _ = _periodic_cells(sites, length)
    return CoveragePlan(
        target_s=sites, target_xy=_target_xy(boundary, sites), cell_bounds=bounds,
        cell_mass=np.bincount(owner, weights=mass, minlength=count),
        h_history=np.asarray(history), max_arc_gap=max_consecutive_arc_gap(sites, length),
        active_count=count, converged=converged, status=status,
        diagnostics={"planner": "weighted_discrete_periodic_lloyd", "density_model": "observed_demand",
                     "confidence_role": "lloyd_gain_only", "quadrature_samples": q_count,
                     "iterations": len(history)-1, "cost_semantics": "discrete_weighted_arc_squared_distance"},
    )
