...
from .radial import BoundaryEstimate, estimate_radial_boundary
...
def build_deployment_boundary(
    ...
    """Explicit polygon-buffer deployment, parametrized by its own arc length.

    The raw estimator and its strict normal-offset adapter are preserved above.
    This separate policy uses the estimated polygon only, not spawn geometry.
    Hole-producing/multi-component buffers remain failures, never silent repairs.
    """
    if not np.isfinite(distance) or distance <= 0:
        raise ValueError("deployment distance must be finite and positive")
    polygon = Polygon(estimate.curve_points).buffer(distance, quad_segs=16)
    if polygon.is_empty or polygon.geom_type != "Polygon" or not polygon.is_valid or polygon.interiors:
        return _failure("OFFSET_INVALID", "buffer_topology_invalid", method="polygon_buffer")
    result = boundary_v2_from_curve(
        np.asarray(polygon.exterior.coords[:-1]), safety_distance=0.0,
        sample_spacing=spacing, method="polygon_buffer_deployment",
        room_size=room_size, room_margin=room_margin,
    )
    if isinstance(result, BoundaryEstimateFailure):
        return result
    # Confidence is interpolated spatially only as planner gain, never risk.
    nearest = np.argmin(np.linalg.norm(
        result.curve_points[:, None] - estimate.curve_points[None, :], axis=2), axis=1)
    return replace(result, confidence=estimate.confidence[nearest],
                   uncertainty=estimate.uncertainty[nearest], diagnostics={
        **result.diagnostics, "deployment_policy": "polygon_buffer",
        "requested_offset": float(distance), "length_semantics": "deployment_curve",
        "estimated_crowd_perimeter": float(estimate.length),
    })
