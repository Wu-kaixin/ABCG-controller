...
## 模型与符号
距离单位为 m、时间为 s。观测是静态中心 $p_j$、半径 $r_j$ 与数值权重 $d_j>0$；权重仅是规划输入，不解释为经验证的行为风险。房间已知，生成多边形对控制器不可见。占据集是观测圆盘的凸包，故对凹点集是保守外包而不是真实边界重建；deployment curve 是该集合的外偏移。代码还验证闭合多边形、人员包含、墙和人员距离。
