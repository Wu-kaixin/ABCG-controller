...
当前实现并严格评价 Step 1：在封闭场地中，围绕一个静态、未知轮廓的人群部署外部引导代理。人群不受引导代理影响；观测为全局快照，通信不受限。当前验收结论为 **PARTIAL**，详见 `docs/step1_validation.md`。
...
每次使用空的结果目录。输出包括严格 JSON 指标、无 pickle 的轨迹数组、逐候选规划历史、任务状态和可选图片。
...
`--workers auto` 使用可用 CPU 数与任务数的较小值；worker 内限制数值库线程。输出目录必须为空，避免把不匹配的旧结果误当 resume。完整冻结矩阵及尚未完成的 resume/固定-N 协议见 `experiments/acceptance_manifest.yaml` 和验收报告。
...
6. 分配使用静态可见图路径长度；速度经过安全过滤并在执行前检查整个采样区间。非法部署、资源必要不足、规划搜索耗尽、路径不可达和超时分别报告。
