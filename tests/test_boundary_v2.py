...
# Analytic fixtures replace the legacy synthetic-crowd runtime dependency.
class StaticCrowdConfig:
    @staticmethod
    def from_dict(raw, seed):
        return {**raw, "seed": seed}


def generate_static_crowd(cfg):
    rng = np.random.default_rng(cfg["seed"])
    angles = rng.uniform(0, 2*np.pi, cfg["count"])
    radii = np.sqrt(rng.uniform(0, 1, cfg["count"]))
    axes = cfg.get("axes", [cfg.get("radius", 1)] * 2)
    return np.asarray(cfg["center"]) + np.column_stack((np.cos(angles), np.sin(angles))) * radii[:, None] * axes
