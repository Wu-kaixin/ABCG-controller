...
from controller.abcg import ABCGController
...
def test_evaluator_recomputes_resource_plan_and_net_clearance():
    b=circle_boundary(); targets=b.deployment[[0,64,128,192]]
    cfg={'hold_steps':1,'position_tolerance':.1,'speed_tolerance':.1,
         'curve_tolerance':.1,'arc_tolerance':0.,'max_gap_limit':9.,
         '_people':np.array([[10.,10.]]),'_radii':np.array([.1]),
         '_room':np.array([20.,20.])}
    trajectory=np.stack([targets,targets]); velocities=np.zeros((1,4,2))
    criteria,diag=evaluate(trajectory,velocities,np.ones(4,bool),targets,
                           np.array([0,0,2,3]),b,Safety(.2,.2,.1,1.),cfg)
    assert not criteria['resource_ok']
    assert set(diag['minimum_net_clearances']) == {'crowd','guides','walls'}


def test_recovery_sequence_is_finite_and_uses_three_distinct_strategies():
    cfg=load_config(ROOT/'configs/step1/square.yaml')
    env=Environment(cfg); observation=env.observe(); guides=env.initialize_guides(cfg['guides'])
    c=cfg['controller']; g=cfg['guides']
    safety=Safety(2*g['radius']+c['clearance'],
                  g['radius']+observation.radii.max()+c['clearance'],
                  g['radius']+c['clearance'],g['max_speed'],c['safety_numeric_tolerance'])
    controller=ABCGController(c,safety,env.scene.size)
    controller.prepare(observation,guides)
    before=controller.remaining_path_lengths(guides)
